...
function Engine:removeEntity(entity, removeChildren, newParent)
  if self.entities[entity.id] then
    ...
    if entity.id then
      ...
      lovetoys.debug(
        "Engine: Entity has not been added to any engine yet. (No entity.id)"
      )
    end
...
function Engine:addSystem(system, type)
  ...
  if type ~= nil and type ~= "draw" and type ~= "update" then
    lovetoys.debug(
      "Engine: Trying to add System " ..
        name .. "with invalid type " .. type .. ". Aborting"
    )
    return
  end
  ...
  if system.draw and system.update and not type then
    lovetoys.debug(
      "Engine: Trying to add System " ..
        name ..
          ", which has an update and a draw function, without specifying type. Aborting"
    )
    return
  end
  ...
  if self.systemRegistry[name] and self.systemRegistry[name] ~= system then
    lovetoys.debug(
      "Engine: Trying to add two different instances of the same system. Aborting."
    )
    return
  end
...
function Engine:registerSystem(system)
  ...
  if system.hasGroups then
    for group, componentList in pairs(system:requires()) do
      ...
      self.singleRequirements[component] =
        self.singleRequirements[component] or {}
