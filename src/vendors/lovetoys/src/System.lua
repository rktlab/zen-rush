  ...
  for group, req in pairs(self:requires()) do
    ...
    if self.hasGroups ~= nil then
      assert(
        ...
        "System " .. self.class.name .. " has mixed requirements in requires()"
      )
    else
      self.hasGroups = requirementIsGroup
    end
...
function System:onAddEntity(entity, group)
end

function System:onRemoveEntity(entity, group)
end
...
function System:pickRequiredComponents(entity)
  ...
  elseif type(lovetoys.util.firstElement(requirements)) == "table" then
    lovetoys.debug(
      "System: :pickRequiredComponents() is not supported for systems with multiple component constellations"
    )
    return nil
  end
