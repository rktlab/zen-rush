...
function Screen:initialize(width, height, scale)
  ...
  return self:setMode(FLOAT):setBackgroundColor(0, 0, 0):setDimensions(
    width,
    height,
    scale
  )
...
function Screen:setMode(mode)
  assert(
    mode == FLOAT or mode == INTEGER,
    'mode must be "float" or "integer", got: ' .. tostring(mode)
  )
  self._mode = mode
  return self
end
...
function Screen:setDimensions(width, height, scale, isSkipWindowResize)
  ...
  return self:setScale(scale, isSkipWindowResize):_saveScale()
end
...
function Screen:increaseScale(isSkipWindowResize)
  return self:setScale(self._scale + 1, isSkipWindowResize):_saveScale()
end
...
function Screen:handleResize()
  ...
  return self:setScale(self:_getMaxScaleForWindow(), skipResize)
end
...
function Screen:windowToScreen(x, y)
  ...
  return (x - self._drawOffsetX) / self._scale, (y - self._drawOffsetY) /
    self._scale
end
...
function Screen:screenToWindow(x, y)
  ...
  return x * self._scale + self._drawOffsetX, y * self._scale +
    self._drawOffsetY
end
