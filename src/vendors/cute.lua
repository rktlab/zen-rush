...
local keypressed = function(key)
  ...
  if show then
    if (key == downKey and offset < #getTests()) then
      offset = offset + 1
    end
    if (key == upKey and offset > 0) then
      offset = offset - 1
    end
  end
end
...
local _drawResultsBox = function(g, w, h)
  ...
  _setColour("black", g)
  g.printf(
    "Cute " .. version .. "- [h] hide [j] scroll up [k] scroll down",
    padding,
    padding + 1,
    w - padding * 2,
    "center"
...
local _drawLine = function(g, i, offset, test)
  ...
  g.print(
    msg,
    padding + margin,
    (padding * 1.5) + margin * 2 + (i - 1 - offset) * 14
  )
end
...
local _is = function(testVal, refVal)
  ...
  if testVal ~= refVal then
    error(tostring(testVal) .. " ~= " .. tostring(refVal), 3)
  end
  return true
end
...
local _shallowMatches = function(testTable, refTable)
  ...
  for k, item in pairs(testTable) do
    if item ~= refTable[k] then
      error(
        "Mismatch for element with key " ..
          tostring(k) ..
            ": " .. tostring(item) .. " ~= " .. tostring(refTable[k]),
        3
      )
    end
  end
...
check = function(testVal)
  return {
    ...
    shallowMatches = function(refTable)
      _shallowMatches(testVal, refTable)
    end
  }
end
